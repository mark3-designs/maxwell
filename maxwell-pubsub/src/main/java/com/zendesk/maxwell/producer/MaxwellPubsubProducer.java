...
import com.zendesk.maxwell.PubSubConfig;
...
public class MaxwellPubsubProducer extends AbstractProducer {
  ...
  private final MaxwellPubsubProducerWorker worker;
	...
	public static final Function<MaxwellContext, AbstractProducer> register() {
		return new Function<MaxwellContext, AbstractProducer>() {
			@Override
			public AbstractProducer apply(MaxwellContext maxwellContext) {
				try {
					return new MaxwellPubsubProducer(maxwellContext,
							((PubSubConfig) maxwellContext.getConfig()).pubsubProjectId,
							((PubSubConfig) maxwellContext.getConfig()).pubsubTopic,
							((PubSubConfig) maxwellContext.getConfig()).ddlPubsubTopic);
...
class MaxwellPubsubProducerWorker
  ...
  public MaxwellPubsubProducerWorker(MaxwellContext context,
    ...
    BatchingSettings.newBuilder()
        .setElementCountThreshold(config().pubsubMessageCountBatchSize)
        .setRequestByteThreshold(config().pubsubRequestBytesThreshold)
        .setDelayThreshold(config().pubsubPublishDelayThreshold)
        .build();
    
    // Retry settings control how the publisher handles retryable failures
    RetrySettings retrySettings =
        RetrySettings.newBuilder()
            .setInitialRetryDelay(config().pubsubRetryDelay)
            .setRetryDelayMultiplier(config().pubsubRetryDelayMultiplier)
            .setMaxRetryDelay(config().pubsubMaxRetryDelay)
            .setInitialRpcTimeout(config().pubsubInitialRpcTimeout)
            .setRpcTimeoutMultiplier(config().pubsubRpcTimeoutMultiplier)
            .setMaxRpcTimeout(config().pubsubMaxRpcTimeout)
            .setTotalTimeout(config().pubsubTotalTimeout)
            .build();
    ...
    if ( config().outputConfig.outputDDL == true &&
  ...
  private PubSubConfig config() {
  	return ((PubSubConfig) context.getConfig());
  }
